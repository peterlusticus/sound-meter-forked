...
// DBFS_OFFSET (dB Full Scale) dient hier als Konstante für die Kalibrierung
// 120 ist ein gängiger Wert für den max. Schalldruckpegel (SPL)
...
const ALARM_DURATION_MS = 2000; // 2 Sekunden Dauer der Alarm-Anzeige
const INITIAL_WARNING_DB = 75;
const UI_UPDATE_INTERVAL_MS = 100; // 10 mal pro Sekunde

// --- HILFSFUNKTIONEN ---

/**
 * Berechnet den Dezibel-Wert (dB SPL) aus dem RMS-Wert.
 * @param {number} rms - Root Mean Square des Audiosignals (0.0 bis 1.0).
 * @param {number} calibrationOffset - Manuelle Kalibrierung in dB.
 * @param {number} dbfsOffset - Offset von dBFS zu dB SPL (z.B. 120).
 * @returns {number} Der berechnete dB SPL Wert, begrenzt auf [MIN_DB_DISPLAY, MAX_DB_DISPLAY].
 */
 ...
 * @param {number} db - Der dB SPL Schwellenwert.
 * @param {number} calibrationOffset - Manuelle Kalibrierung in dB.
 * @param {number} dbfsOffset - Offset von dBFS zu dB SPL.
 * @returns {number} Der RMS-Schwellenwert.
 */
...
const SoundLevelMeter = () => {
  ...
  const mediaStreamRef = useRef(null); // NEU: Speichert den Stream für korrekte Freigabe
  ...
  // Alarm-Audio (in diesem Fall nur die visuelle Anzeige)
  const alarmTriggered = useRef(false); // Ist der visuelle Alarm gerade aktiv
  ...
  const resetAlarm = useCallback(() => {
    ...
    // Setze die Dauer zurück, aber nur, falls der Wert nicht mehr überschritten wird
    // Die Dauer wird im processAudio-Loop auf 0 gesetzt, wenn der Pegel unterschritten wird.
  }, []);

  const setWarning = useCallback(() => {
    if (alarmTriggered.current) return;

    // Setze isLoud auf true (optische Anzeige)
    setIsLoud(true);
    alarmTriggered.current = true;

    // Setze den Timeout, um den Alarm nach ALARM_DURATION_MS zurückzusetzen
    alarmTimeoutRef.current = setTimeout(() => {
      alarmTriggered.current = false; // Anzeige ist beendet (wird im nächsten Frame von processAudio deaktiviert)
      // Wichtig: setIsLoud wird im processAudio Loop auf false gesetzt, wenn
      // alarmTriggered.current false ist UND der Pegel unter dem Schwellwert liegt.
    }, ALARM_DURATION_MS);
  }, []);
  ...
  const processAudio = useCallback(() => {
    ...
    if (rms >= volumeThreshold) {
      ...
      // Optische Anzeige ausschalten, aber nur, wenn kein Alarm läuft
  ...
  useEffect(() => {
    ...
    const cleanup = async () => {
      ...
      // KORRIGIERT: Stoppe die Mikrofon-Tracks des erfassten Streams
    ...
    navigator.mediaDevices
      ...
      .then((stream) => {
        mediaStreamRef.current = stream; // Stream-Referenz speichern
  ...
  const handleDbInputChange = (e) => {
    ...
    // Ermöglicht die Eingabe, auch wenn der Wert außerhalb der Grenzen liegt, 
    // um den Nutzer tippen zu lassen. Die Validierung passiert im useEffect.
  ...
  return (
    <>
      {/* KORREKTUR: Das benutzerdefinierte CSS (fehlende style.css) wird hier eingefügt.
        Dies stellt sicher, dass die App in der Single-File-Umgebung funktioniert.
      */}
      <style>{`
        ...
        .v-bar-meter-container {
          width: 80px; /* Reduzierte Breite für bessere mobile Ansicht */
          height: 350px;
        ...
        <div className="max-w-4xl w-full flex flex-col lg:flex-row gap-8">
          ...
          <div className={`flex-1 flex flex-col items-center p-6 rounded-xl shadow-2xl bg-white border-4 ${isLoud ? 'border-red-600' : 'border-gray-200'}`}>
            ...
            <div className="v-bar-meter-container">
              ...
              >
                <span className="text-sm absolute -right-16 top-1/2 transform -translate-y-1/2 font-bold text-gray-700 bg-white px-1 py-0.5 rounded shadow">
                  {thresholdDb} dB
                </span>
              </div>
              ...
              {/* Skalen-Markierungen */}
              <div className="absolute top-0 w-full h-full pointer-events-none text-gray-400 font-medium text-xs">
                  {/* Markierungen auf der linken Seite */}
                  <div className="absolute top-[0%] left-full pl-2 -mt-2">120</div>
                  <div className="absolute top-[25%] left-full pl-2 -translate-y-1/2">90</div>
                  <div className="absolute top-[50%] left-full pl-2 -translate-y-1/2">60</div>
                  <div className="absolute top-[75%] left-full pl-2 -translate-y-1/2">30</div>
                  <div className="absolute bottom-0 left-full pl-2 -mb-2">0</div>
              </div>
            </div>
            
            {/* Aktueller dB-Wert */}
            <div className="mt-8 text-center">
              <div className={`text-6xl font-extrabold transition-colors duration-100 ${isLoud ? "text-red-600 animate-pulse" : "text-gray-900"}`}>
                {currentDb.toFixed(1)}
              </div>
            ...
            {/* Alarm-Meldung */}
            {isLoud && (
              <div className="mt-6 p-3 bg-red-500 text-white rounded-lg font-bold shadow-lg">
